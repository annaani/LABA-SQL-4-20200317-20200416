...
ORDER BY margin DESC, prod.product_name;
...
SELECT
 ...
 --  CAST((prod.list_price / prod.standard_cost -1)  * 100 as decimal(10,2) )+ '%' as margin_to_cost_4_proc
FROM
	db_laba.dbo.products prod
order by
	1;
...
SELECT
	...
	--,reverse(emp.last_name)
	--,reverse(SUBSTRING ( reverse (emp.last_name) , 1 ,3 ))
	--,LEN(emp.last_name) 
	--,LTRIM (emp.last_name)
	--,RTRIM (emp.last_name)
	--,TRIM (emp.last_name)
	...
	,SUBSTRING ( emp.last_name , CHARINDEX('ash', emp.last_name) ,3 )
...
from
	db_laba.dbo.employees as emp1
left join db_laba.dbo.employees as emp2 on
--join db_laba.dbo.employees emp2 on
	emp1.manager_id = emp2.employee_id
--inner lfkgflkgjklfjglk  on 
--fdjkghfdjg = kjsfkjgshfg
order by 1,2;
...
FULL join db_laba.dbo.orders ord on
